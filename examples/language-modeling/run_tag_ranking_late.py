...
from transformers.modeling_bert import BertConfig, BertForTagRankingLate
...
class DataCollatorForTagGeneration:
    ...
    def collate_batch(self, examples):
        if 'labels' in examples[0]:
            ...
            image_ids = torch.tensor([e['image_ids'] for e in examples], dtype=torch.long)
            location_ids = torch.tensor([e['location_ids'] for e in examples], dtype=torch.long)
            time_ids = torch.tensor([e['time_ids'] for e in examples], dtype=torch.long)
            text_ids = torch.tensor([e['text_ids'] for e in examples], dtype=torch.long)
            
            return {'image_ids': image_ids, 'location_ids': location_ids, 'time_ids': time_ids, 'text_ids': text_ids, 'labels': labels}
        else:
            return examples[0]
...
def convert_example_to_feature(example, tokenizer, block_size):
    ...
    image = tokenizer.tokenize(image)[:max_size]
    location = tokenizer.tokenize(location)[:max_size]
    time = tokenizer.tokenize(time)[:max_size]
    text = tokenizer.tokenize(text)[:max_text_size]

    image_input = [tokenizer.cls_token] + [tokenizer.pad_token] * (block_size - 1)
    location_input = [tokenizer.cls_token] + [tokenizer.pad_token] * (block_size - 1)
    time_input = [tokenizer.cls_token] + [tokenizer.pad_token] * (block_size - 1)
    text_input = [tokenizer.cls_token] + [tokenizer.pad_token] * (block_size - 1)

    image_input[3 * max_size + max_text_size + 1] = tokenizer.sep_token
    location_input[3 * max_size + max_text_size + 1] = tokenizer.sep_token
    time_input[3 * max_size + max_text_size + 1] = tokenizer.sep_token
    text_input[3 * max_size + max_text_size + 1] = tokenizer.sep_token

    image_input[1+(0*max_size):len(image)+1+(0*max_size)] = image
    location_input[1+(1*max_size):len(location)+1+(1*max_size)] = location
    time_input[1+(2*max_size):len(time)+1+(2*max_size)] = time
    text_input[1+(3*max_size):len(text)+1+(3*max_size)] = text

    image_ids = tokenizer.convert_tokens_to_ids(image_input)
    location_ids = tokenizer.convert_tokens_to_ids(location_input)
    time_ids = tokenizer.convert_tokens_to_ids(time_input)
    text_ids = tokenizer.convert_tokens_to_ids(text_input)
    
    if label is None: # For evaluation
        return {'pid': example['pid'], 'image_ids': image_ids, 'location_ids': location_ids, 'time_ids': time_ids, 'text_ids': text_ids}
...
def main():
    ...
    if training_args.do_eval:
        ...
        for eid, example in enumerate(tqdm(dataloader, desc=description)):
            ...
            image_ids = torch.tensor([feature['image_ids']], dtype=torch.long).to(training_args.device)
            location_ids = torch.tensor([feature['location_ids']], dtype=torch.long).to(training_args.device)
            time_ids = torch.tensor([feature['time_ids']], dtype=torch.long).to(training_args.device)
            text_ids = torch.tensor([feature['text_ids']], dtype=torch.long).to(training_args.device)
            pid = feature['pid']
            inputs = {'image_ids': image_ids, 'location_ids': location_ids, 'time_ids': time_ids, 'text_ids': text_ids}
            with torch.no_grad():
